...
class CorrespondenceController extends Controller
{
    ...
    //         }
    //     }
    //     $projects = Project::all(); // أو أي منطق خاص بك
    //     return view('correspondence.index', compact('correspondences', 'statistics', 'subDepartments', 'projects'));
    //     // التصفية حسب الحالة
    ...
    //     $correspondences = $query->paginate($request->get('per_page', 20));
    ...
    // }
    /**
     * Display a listing of correspondences with advanced filtering.
     */
    public function index(Request $request)
    {
        // 1. الصلاحية
        ...
        // 2. الاستعلام الأساسي مع العلاقات المطلوبة
        // (DomainScope يطبق تلقائياً فلترة النطاق الجغرافي والإداري)
        $query = Correspondence::with([
            'senderEntity',
            'recipientEntity',
            'senderUser',
            'project',                 // للفلترة وعرض اسم المشروع
            'latestReferral.referredToEntity',          // لأحدث إحالة
            'referrals',               // لعدد الإحالات
        ]);

        // 3a. فلتر "الكل / المرسلة / الواردة / المحالة / المتأخرة" إلخ
        ...
        // 3b. فلتر الحالة (status)
        if ($request->filled('status')) {
            $query->where('status', $request->status);
        }

        // 3c. فلتر الأولوية (priority)
        if ($request->filled('priority')) {
            $query->where('priority', $request->priority);
        }

        // 3d. فلتر السرية (confidential)
        if ($request->has('confidential')) {
            $query->where('confidential', $request->boolean('confidential'));
        }

        // 3e. فلتر التاريخ (من - إلى)
        ...
        if ($request->filled('search')) {
            ...
            $query->where(function ($q) use ($search) {
                $q->where('subject', 'like', "%{$search}%")
                    ...
                    ->orWhere('message_body', 'like', "%{$search}%")
                    ->orWhere('notes', 'like', "%{$search}%")
                    ->orWhereHas('senderEntity', fn ($q) => $q->where('name', 'like', "%{$search}%"))
                    ->orWhereHas('recipientEntity', fn ($q) => $q->where('name', 'like', "%{$search}%"));
            });
        }

        // ------------------------------------------------------------
        // 4. الفلاتر الخاصة بالإحالات (referral_status, referred_to_department)
        // ------------------------------------------------------------
        if ($request->filled('referral_status')) {
            $query->whereHas('latestReferral', function ($q) use ($request) {
                $q->where('referral_status', $request->referral_status);
            });
        }

        if ($request->filled('referred_to_department')) {
            $query->whereHas('latestReferral', function ($q) use ($request) {
                $q->where('referred_to_entity_id', $request->referred_to_department);
            });
        }

        // ------------------------------------------------------------
        // 5. الفلاتر الجديدة حسب المشروع (لتبويبات "المشاريع" و "مواضيع خارجية")
        // ------------------------------------------------------------
        if ($request->filled('has_project')) {
            switch ($request->has_project) {
                case 'yes':
        ...
        // فلتر حسب مشروع محدد (للاختيار من القائمة المنسدلة)
        ...
        $allowedSorts = ['created_at', 'status', 'priority', 'subject', 'correspondence_number'];
        ...
        // ------------------------------------------------------------
        // 7. تنفيذ الاستعلام مع التصفح (paginate)
        // ------------------------------------------------------------
        $perPage = in_array((int) $request->get('per_page'), [20, 50, 100, 500])
            ? (int) $request->get('per_page')
            : 20;
        $correspondences = $query->paginate($perPage)->appends($request->except('page'));

        // ------------------------------------------------------------
        // 8. الإحصائيات (KPIs) – المطلوبة في بطاقات الواجهة
        // ------------------------------------------------------------
        // نطاق المراسلات التي تخص الجهة الحالية
        $baseQuery = Correspondence::where(function ($q) use ($entityId) {
            $q->where('sender_entity_id', $entityId)
                ->orWhere('recipient_entity_id', $entityId);
        });

        $totalCorrespondences = (clone $baseQuery)->count();
        $totalProjects = Project::count(); // أو استخدم Project::whereIn('id', ...) حسب نطاق الجهة
        $pendingReferrals = CorrespondenceReferral::whereIn('correspondence_id', (clone $baseQuery)->select('id'))
            ->where('referral_status', 'pending')
            ->count();
        $completedReferrals = CorrespondenceReferral::whereIn('correspondence_id', (clone $baseQuery)->select('id'))
        ...
        $statistics = [
            'total_correspondences' => $totalCorrespondences,
            'total_projects' => $totalProjects,
            'pending_referrals' => $pendingReferrals,
            'completed_referrals' => $completedReferrals,
            'overdue' => $overdue,
            // يمكن الاحتفاظ بالمفاتيح القديمة إذا استخدمها مكان آخر في الـ View
            'total_topics' => $totalCorrespondences,
            'total_received' => (clone $baseQuery)->where('recipient_entity_id', $entityId)->count(),
            'pending' => (clone $baseQuery)->where('recipient_entity_id', $entityId)->where('status', 'pending')->count(),
        ];
    ...
    public function getMovementTimeline($id)
    {
        ...
        $timeline = $correspondence->getFullMovementTimeline();
    ...
    public function print($id)
    {
        ...
        if (! empty($html) && is_string($html)) {
            ...
            set_error_handler(function ($errno, $errstr) {
                if (
                    strpos($errstr, 'Uninitialized string offset 0') !== false ||
    ...
    /**
     * معاينة المراسلة قبل الطباعة (داخل iframe)
     * تعرض نفس قالب الطباعة كـ HTML بدون توليد PDF
     */
    public function preview($id)
    {
        ...
        // عرض القالب كـ HTML للمعاينة داخل iframe (بدون رأس وتذييل الصفحة)
        return view('correspondence.print', $pdfData);
    }
    ...
    public function edit($id)
    {
        ...
        $user = Auth::user();
        $type = $correspondence->correspondence_type ?? 'new';
        ...
        return view('correspondence.form', compact('correspondence', 'entities', 'type', 'parent', 'senderEntity'));
    }
    ...
    public function overdue(Request $request)
    {
        ...
        $correspondences = $query->orderBy('created_at', 'desc')->paginate(20);
    ...
    public function restore($id)
    {
        ...
        try {
            ...
            return redirect()->route('correspondence.show', $correspondence->id)
                ->with('success', 'تم استعادة المراسلة بنجاح.');
    ...
    public function acknowledgeForward($id)
    {
        ...
        return redirect()->back()->with('success', 'تم تأكيد استلام التوجيه بنجاح.');
    }

    /**
     * Search projects for correspondence linkage
     * Returns all projects of all types as requested by user
     */
    public function searchProjects(Request $request)
    {
        ...
        // إرجاع جميع المشاريع بدون فلترة حسب الجهة بناءً على طلب المستخدم
        $projects = $projectsQuery->limit(50)->get()
            ->map(fn ($p) => ['id' => $p->id, 'text' => $p->project_name.' ('.$p->form_number.')']);
