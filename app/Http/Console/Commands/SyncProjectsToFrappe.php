...
{
    protected $signature = 'frappe:sync-projects {--force : Force resync of already synced projects} {--dry-run : Show what would be synced without actually syncing}';

    protected $description = 'Automatically POST projects in execution phase to Frappe API. Syncs new projects and retries failed ones.';
    ...
    public function handle(): int
    {
        ...
        if (! $this->verifyFrappeConnection()) {
            ...
            Log::error('Frappe sync failed: API connection error');
        ...
        $this->syncProjects($force, $dryRun);
    ...
    private function syncProjects(bool $force, bool $dryRun): void
    {
        $query = Project::where('status', 'in_progress');

        if (! $force) {
            $query->where('frappe_synced', false);
        }
    ...
    {
        $this->line('', 'info');
        $this->info("📋 Syncing: {$project->project_name} (ID: {$project->id})");
        ...
        if ($result['success']) {
            $project->update([
                ...
                'frappe_project_id' => $result['frappe_id'] ?? null,
                'frappe_sync_error' => null,
            ]);

            $this->line('   ✅ Synced successfully');
            $this->line("   Frappe ID: {$result['frappe_id']}", 'info');

            Log::info('Project synced to Frappe successfully', [
                'project_id' => $project->id,
                'frappe_id' => $result['frappe_id'],
    ...
    private function logResults(): void
    {
        ...
        $this->info('📝 Detailed logs written to: storage/logs/frappe_sync.log');
    }
}
