 ...
 */
(function () {
if (window.HijriConverter) {
    window.ExecutiveActionRowInitializer = window.HijriConverter.initializeActionCard
        ? window.HijriConverter.initializeActionCard.bind(window.HijriConverter)
        : window.ExecutiveActionRowInitializer;
    return;
}
...
class HijriConverter {
    ...
    static initializeActionCard(card) {
        // Classes used in Executive and Preliminary tables
        const startDateInput = card.querySelector('.executive-start-date, .procedure-start-date');
        const startHijriInput = card.querySelector('.executive-start-date-hijri, .procedure-start-date-hijri');
        const endDateInput = card.querySelector('.executive-end-date, .procedure-end-date');
        const endHijriInput = card.querySelector('.executive-end-date-hijri, .procedure-end-date-hijri');
        const durationInput = card.querySelector('.executive-duration, .procedure-duration');

        const updateDatesAndDuration = () => {
        ...
        if (startDateInput) {
            startDateInput.addEventListener('input', updateDatesAndDuration);
            startDateInput.addEventListener('change', updateDatesAndDuration);
        }
        if (endDateInput) {
            endDateInput.addEventListener('input', updateDatesAndDuration);
            endDateInput.addEventListener('change', updateDatesAndDuration);
        }
    ...
    /**
     * Automated Global Initializer for date pairs with data attributes.
     * Example: <input type="date" data-hijri-target="#hijri_input">
     */
    static autoBind() {
        document.querySelectorAll('input[type="date"][data-hijri-target]').forEach(gregInput => {
            const hijriTarget = document.querySelector(gregInput.dataset.hijriTarget);
            if (hijriTarget) {
                const update = () => {
                    const hijri = this.gregorianToHijri(gregInput.value);
                    hijriTarget.value = this.formatHijri(hijri);
                };
                gregInput.addEventListener('input', update);
                gregInput.addEventListener('change', update);
                hijriTarget.readOnly = true;
                update();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Activities and Implementation rows
    document.querySelectorAll('.executive-activity-action-row, .procedure-row').forEach(card => {
