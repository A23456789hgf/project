                ...
                <form action="{{ route('auth.login') }}" method="POST" id="loginForm" novalidate>
                    ...
                    <div class="custom-input-group">
                        ...
                        <div class="input-wrapper">
                            <input type="text" id="phone" name="phone" class="custom-form-control"
                                ...
                                autocomplete="tel">
                            <x-icon name="phone" class="input-icon" />
                        </div>
                        @error('phone')
                            <div class="error-msg" role="alert">
                                <x-icon name="exclamation-circle" style="width: 12px; height: 12px;" />
                                {{ $message }}
                            </div>
                        @enderror
                    </div>
                    ...
                    <div class="custom-input-group">
                        ...
                        <div class="input-wrapper">
                            ...
                            <button type="button" class="password-toggle" onclick="togglePassword()"
                                ...
                                <x-icon name="eye" id="toggleIconEye" style="width: 14px; height: 14px;" />
                                <x-icon name="eye-slash" id="toggleIconEyeSlash"
                    ...
                    <div class="actions-row">
                        ...
                        <a href="#" class="forgot-link" data-bs-toggle="modal"
                            data-bs-target="#forgotPasswordModal">نسيت كلمة المرور؟</a>
                    </div>

                    <button type="submit" class="submit-btn" id="submitBtn">
                        <div class="spinner" id="btnSpinner"></div>
                        <span id="btnText">تسجيل الدخول</span>
                        <x-icon name="arrow-left" id="btnIcon" style="width: 14px; height: 14px;" />
                    </button>
                </form>
        ...
        <div class="brand-section">
            ...
            <div class="brand-content">
                <div class="logo-container">
                    <img src="{{ asset('images/logo.png') }}" alt="شعار وزارة الزراعة والثروة السمكية والموارد المائية">
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                <div class="modal-header">
                    ...
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="إغلاق" style="margin: 0; margin-left: auto;"></button>
                </div>
                    ...
                    <div id="step-phone">
                        ...
                        <div class="d-grid mt-3">
                            <button type="button" class="btn btn-primary" id="btn-send-otp">
                                ...
                                <div class="spinner-border spinner-border-sm text-light ms-2" id="spinner-send-otp" role="status" style="display: none;"></div>
                            </button>
                        </div>
                    </div>
                    ...
                    <div id="step-otp" style="display: none;">
                        ...
                        <div class="d-grid mt-3">
                            <button type="button" class="btn btn-primary" id="btn-verify-otp">
                                ...
                                <div class="spinner-border spinner-border-sm text-light ms-2" id="spinner-verify-otp" role="status" style="display: none;"></div>
                            </button>
                        </div>
                    </div>
                    ...
                    <div id="step-new-password" style="display: none;">
                        ...
                        <div class="d-grid mt-3">
                            <button type="button" class="btn btn-primary" id="btn-reset-password">
                                ...
                                <div class="spinner-border spinner-border-sm text-light ms-2" id="spinner-reset-password" role="status" style="display: none;"></div>
                            </button>
                        </div>
                    </div>
            ...
            btnSendOtp.addEventListener('click', async function () {
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        showAlert(data.message || data.errors?.phone?.[0] || 'حدث خطأ');
                    }
                } catch (error) { showAlert('حدث خطأ في الاتصال'); }
                finally { toggleLoading('send-otp', false); }
            });
            ...
            btnVerifyOtp.addEventListener('click', async function () {
                ...
                try {
                    ...
                    if (response.ok && data.success) {
                        ...
                        showAlert(data.message || data.errors?.otp?.[0] || 'رمز غير صحيح');
                    }
                } catch (error) { showAlert('حدث خطأ في الاتصال'); }
                finally { toggleLoading('verify-otp', false); }
            });
            ...
            btnResetPassword.addEventListener('click', async function () {
                ...
                try {
                    const response = await fetch('{{ route("password.otp.reset") }}', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json', 'X-CSRF-TOKEN': csrfToken, 'Accept': 'application/json' },
                        body: JSON.stringify({ phone: phone, otp: otp, password: newPassword, password_confirmation: confirmPassword })
                    });
                    const data = await response.json();
                    if (response.ok && data.success) {
                        showAlert(data.message + '. سيتم التحديث...', 'success');
                        setTimeout(() => window.location.reload(), 2000);
                    } else {
                        showAlert(data.message || data.errors?.password?.[0] || 'حدث خطأ');
                    }
                } catch (error) { showAlert('حدث خطأ في الاتصال'); }
                finally { toggleLoading('reset-password', false); }
            });
