                    ...
                    <td>
                        @if($movement->user)
                            <div class="d-flex align-items-center">
                                <x-icon name="user-circle" class="me-1 text-muted" />
                        ...
                        @if($movement->from_entity || $movement->to_entity)
                            <div class="small">
                                @if($movement->from_entity)
                                    <div><x-icon name="sign-out-alt" class="text-danger me-1" />من: {{ $movement->from_entity }}</div>
                                @endif
                                @if($movement->to_entity)
                                    <div><x-icon name="sign-in-alt" class="text-success me-1" />إلى: {{ $movement->to_entity }}</div>
                                @endif
                            </div>
                        @else
