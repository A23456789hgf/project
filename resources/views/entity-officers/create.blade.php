    ...
    <div class="main-card shadow-lg border-0 rounded-4">
        <div class="page-header mb-4 p-4 bg-light rounded-4">
            <h2 class="fw-bold mb-0" style="color: #001f3f;">
                <i class="fas fa-plus-circle text-success me-2"></i> إضافة مسؤول جديد
            </h2>
            <div class="title-line mt-2"></div>
        </div>

        <form action="{{ route('entity-officers.store') }}" method="POST" class="p-4">
            @csrf
            ...
            <div class="row g-4">
                ...
                <div class="col-md-12">
                    ...
                    <div class="d-flex gap-5">
                        <div class="form-check custom-radio">
                            <input class="form-check-input" type="radio" name="entity_type" id="type_internal" value="internal" {{ old('entity_type', 'internal') === 'internal' ? 'checked' : '' }}>
                        ...
                        <div class="form-check custom-radio">
                            <input class="form-check-input" type="radio" name="entity_type" id="type_external" value="external" {{ old('entity_type') === 'external' ? 'checked' : '' }}>
                ...
                {{-- الجهة (داخلية) --}}
                <div class="col-md-6" id="internal_entity_wrapper">
                    ...
                    <select name="internal_entity_id" id="internal_entity_id" class="form-select select2-enable @error('internal_entity_id') is-invalid @enderror">
                        ...
                        @foreach($internalEntities as $entity)
                            <option value="{{ $entity->id }}" {{ old('internal_entity_id') == $entity->id ? 'selected' : '' }}>
                                {{ $entity->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- الجهة (خارجية) --}}
                <div class="col-md-6 d-none" id="authority_wrapper">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="admin_name" id="admin_name" class="form-control custom-field @error('admin_name') is-invalid @enderror" required value="{{ old('admin_name') }}" placeholder="أدخل اسم المسؤول الكامل">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="job_title" id="job_title" class="form-control custom-field @error('job_title') is-invalid @enderror" required value="{{ old('job_title') }}" placeholder="أدخل المسمى الوظيفي (مثال: مدير عام)">
                    @error('job_title') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
            </div>

            <div class="mt-5 pt-3 border-top d-flex gap-3">
                <button type="submit" class="btn btn-primary px-5 rounded-3 fw-bold shadow-sm hover-lift">
...
$(document).ready(function() {
    ...
    $('input[name="entity_type"]').change(function() {
        ...
        if (type === 'internal') {
            ...
            // Clear selections in the hidden field
            $('#authority_id').val(null).trigger('change');
        } else {
            $('#internal_entity_wrapper').addClass('d-none');
            $('#authority_wrapper').removeClass('d-none');
            // Clear selections in the hidden field
            $('#internal_entity_id').val(null).trigger('change');
        }
    });

    // Trigger change on load to handle validation errors state
    $('input[name="entity_type"]:checked').trigger('change');
});
</script>
<style>
.hover-lift {
