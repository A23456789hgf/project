...
@section('title', 'تفاصيل الاستشارة: ' . ($project->project_name ?? 'مشروع #' . $referral->project_id))

@section('styles')
<style>
    .consultation-show-header {
    ...
    .timeline-item.dark .timeline-icon {
        border-color: #1e293b;
        color: #1e293b;
...
<div class="container-fluid px-3 px-md-4 py-3">
    ...
    {{-- Top Banner Header --}}
    <div class="consultation-show-header">
        <div class="d-flex flex-column flex-md-row align-items-md-center justify-content-between gap-3">
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-2 text-white-50 small">
                        ...
                        <li class="breadcrumb-item"><a href="{{ route('consultations.index') }}" class="text-white-50 text-decoration-none">الاستشارات والإحالات</a></li>
                        <li class="breadcrumb-item active text-white" aria-current="page">تفاصيل الاستشارة</li>
                    </ol>
                </nav>
                <div class="d-flex align-items-center gap-3 flex-wrap">
                    <h3 class="fw-bold mb-0 text-white">
                        {{ $project->project_name ?? 'مشروع #' . $referral->project_id }}
                    </h3>
                    ...
                    @elseif($referral->status === 'closed')
                        <span class="badge bg-dark text-white px-3 py-1 rounded-pill fw-bold">
            ...
            <div class="d-flex gap-2 flex-wrap">
                <a href="{{ route('consultations.index') }}" class="btn btn-outline-light rounded-pill px-4 btn-sm fw-semibold">
    ...
    <div class="row g-4">
        ...
        <div class="col-12 col-xl-8">
                ...
                <div class="card-body p-4">
                    <div class="row g-3 mb-3">
                        <div class="col-12 col-md-6">
                            <div class="info-label">الجهة المُحيلة</div>
                            <div class="info-value text-dark d-flex align-items-center gap-2">
                                <i class="fas fa-building text-secondary"></i>
                                <span>{{ $referral->referringEntity->name ?? 'غير محدد' }}</span>
                            </div>
                        </div>
                        <div class="col-12 col-md-6">
                            <div class="info-label">الجهة المُستشارَة</div>
                            <div class="info-value text-primary d-flex align-items-center gap-2">
                                <i class="fas fa-building text-primary"></i>
                                <span>{{ $referral->referredEntity->name ?? 'غير محدد' }}</span>
                            </div>
                        </div>
                        <div class="col-12 col-md-6">
                            ...
                            <div class="info-value d-flex align-items-center gap-2">
                                ...
                                <span>{{ $referral->referringUser->name ?? 'غير محدد' }}</span>
                            </div>
                        </div>
                        <div class="col-12 col-md-6">
                    ...
                    <div class="mb-3">
                        <label class="info-label">نص الاستشارة / المطلوب من الجهة المستشارة:</label>
            ...
            <div class="card tracker-card">
                <div class="card-header bg-white border-bottom p-4">
                    <h5 class="fw-bold mb-0 text-dark">
                        <i class="fas fa-history text-primary me-2"></i> سجل وتاريخ الاستشارة (Timeline)
                    </h5>
                </div>
                <div class="card-body p-4">
                    <div class="referral-timeline">
                            ...
                            <div class="card border-0 shadow-sm bg-light">
                                <div class="card-body p-3">
                                    ...
                                    <div class="small text-muted mb-2">
                                        تمت الإحالة من قبل <strong>{{ $referral->referringUser->name ?? 'المستخدم' }}</strong> ({{ $referral->referringEntity->name ?? '' }}) إلى <strong>{{ $referral->referredEntity->name ?? '' }}</strong>.
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {{-- Step 3: Closed if status is closed --}}
                        @if($referral->status === 'closed')
                            <div class="timeline-item dark">
                                <div class="timeline-icon"><i class="fas fa-lock"></i></div>
                                <div class="card border-0 shadow-sm bg-dark bg-opacity-10 border-dark">
                                    <div class="card-body p-3">
                                        <h6 class="fw-bold mb-1 text-dark">تم إغلاق طلب الاستشارة</h6>
                                        <div class="small text-muted">
                                            تم اعتماد الإفادة وإغلاق الاستشارة بصورة نهائية.
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <div class="col-12 col-xl-4">
            <div class="card tracker-card position-sticky" id="response-board" style="top: 85px;">
                <div class="card-header bg-white border-bottom p-4">
                    <h5 class="fw-bold mb-0 text-dark">
                    ...
                    @if($referral->status === 'pending')
                        @if($canRespond)
                            <div class="alert alert-primary border-0 rounded-3 mb-4 p-3">
                                ...
                                <p class="small mb-0">يرجى دراسة تفاصيل الاستشارة والمرفقات وكتابة الرأي والملاحظات الفنية أدناه.</p>
                            </div>

                            <form id="referralResponseForm" action="{{ route('consultations.respond', $referral) }}" method="POST" enctype="multipart/form-data">
                                @csrf
                        ...
                        @else
                            <div class="text-center py-4">
                                ...
                                <p class="text-muted small mb-0">
                                    هذه الاستشارة موجهة إلى <strong>{{ $referral->referredEntity->name ?? 'الجهة المستشارة' }}</strong>. فقط المستخدمون المخولون من تلك الجهة يحق لهم الرد.
                                </p>
                            </div>
                        @endif
                    @else
                        <div class="text-center py-4">
                            ...
                            <p class="text-muted small mb-3">
                                تم تقديم الإفادة المطلوبة بنجاح.
                            </p>

                            @if($canClose && $referral->status !== 'closed')
                                <form action="{{ route('consultations.close', $referral) }}" method="POST" class="mb-3">
                                    @csrf
                                    <input type="hidden" name="notes" value="تم تأكيد استلام الرد وإغلاق الاستشارة">
                                    <button type="submit" class="btn btn-outline-dark rounded-pill btn-sm px-4 fw-bold">
                                        <i class="fas fa-lock me-1"></i> إغلاق الاستشارة نهائياً
                                    </button>
                                </form>
                            @endif
