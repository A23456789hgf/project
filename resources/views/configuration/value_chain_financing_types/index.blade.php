...
@section('content')
    ...
    @cannot('value-chain-financing-types.view')
        ...
        <div class="main-card p-4 bg-white rounded-4 shadow-sm border-0">
            ...
            <div class="page-header d-flex justify-content-between align-items-center mb-4">
                ...
                <div class="d-flex gap-2">
                    {{-- Create --}}
                    @can('value-chain-financing-types.create')
                        <a href="{{ route('value-chain-financing-types.create') }}"
                            class="btn btn-success d-flex align-items-center gap-2 px-4 rounded-3 fw-bold shadow-sm auth-perm-value-chain-financing-types-create">
                    ...
                    {{-- Data Dropdown --}}
                    @canany(['value-chain-financing-types.export', 'value-chain-financing-types.import'])
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu dropdown-menu-end shadow-lg border-0 rounded-3 mt-2">
                                {{-- Export --}}
                                @can('value-chain-financing-types.export')
                                    <li>
                                        <a class="dropdown-item py-2 d-flex align-items-center gap-2 auth-perm-value-chain-financing-types-export"
                                            href="{{ route('config.export', ['entity' => 'value-chain-financing-types']) }}">
                                ...
                                {{-- Import --}}
                                @can('value-chain-financing-types.import')
                                    @can('value-chain-financing-types.export')
                                        <li>
                                    ...
                                    @endcan
                                    <li>
                                        <a class="dropdown-item py-2 d-flex align-items-center gap-2 auth-perm-value-chain-financing-types-import"
                                            href="{{ route('config.import', ['entity' => 'value-chain-financing-types']) }}">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 512 512"
                                                fill="#0ea5e9">
                                                <path
                ...
                <table class="table custom-table w-100">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">اسم النوع</th>
                            <th class="text-start">أنشئ بواسطة</th>
                            <th>الحالة</th>
                            <th>تاريخ الإنشاء</th>
                            @canany(['value-chain-financing-types.edit', 'value-chain-financing-types.delete'])
                    ...
                    <tbody>
                        @forelse($types as $type)
                            <tr>
                                ...
                                <td>
                                    @if($type->creator_username)
                                        <div class="d-flex flex-column">
                                            <span class="fw-bold text-dark">{{ $type->creator_username }}</span>
                                            <small class="text-muted" style="font-size: 0.8rem;">
                                                {{ $type->creatorEntity->name ?? '-' }}
                                            </small>
                                        </div>
                                    @else
                                ...
                                @canany(['value-chain-financing-types.edit', 'value-chain-financing-types.delete'])
                                    <td>
                                        <div class="d-flex justify-content-center gap-2">
                                            {{-- Edit --}}
                                            @can('value-chain-financing-types.edit')
                                                <a href="{{ route('value-chain-financing-types.edit', $type->id) }}"
                                                    class="btn-action btn-edit auth-perm-value-chain-financing-types-edit" title="تعديل">
                                                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 512 512"
                                                        fill="currentColor">
                                                        <path
                                            ...
                                            {{-- Delete --}}
                                            @can('value-chain-financing-types.delete')
                                                <form action="{{ route('value-chain-financing-types.destroy', $type->id) }}" method="POST"
                                                    class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn-action btn-delete auth-perm-value-chain-financing-types-delete"
