        ...
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2>
                    ...
                    مراجعة بيانات أنواع تمويل السلاسل
                </h2>
            ...
            <div class="card import-preview-card mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('value-chain-financing-types.import.process') }}" method="POST" id="importForm" class="import-form">
                        @csrf
                                        ...
                                        <td>
                                            <select name="mapping[{{ $field }}]" class="form-select" {{ in_array($field, ['name']) ? 'required' : '' }}>
                                                ...
                                                @foreach($headers as $header)
                                                    <option value="{{ $header }}" 
                                                        @if(
                                                            ($field === 'name' && (str_contains(strtolower($header), 'name') || str_contains(strtolower($header), 'اسم') || str_contains(strtolower($header), 'نوع'))) ||
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('importForm').addEventListener('submit', function(e) {
        ...
        if (!nameMapping) {
            e.preventDefault();
            alert('يجب ربط حقل "اسم النوع" بعمود من ملف Excel');
            return false;
        }
    });
});
