...
<div class="container-fluid px-5 py-4">
    <div class="page-header-box">
        <h2 class="page-main-title">تعديل نوع التقرير</h2>
        <p class="text-muted small">تعديل بيانات: <strong>{{ $reportType->name }}</strong></p>
        <div class="section-divider"></div>
    </div>

    <form action="{{ route('report-types.update', $reportType->id) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="row g-4 align-items-end">
            ...
            <div class="col-md-6">
                ...
                <select name="is_active" id="is_active" class="form-select custom-field">
                    <option value="1" {{ old('is_active', $reportType->is_active ? '1' : '0') == '1' ? 'selected' : '' }}>مفعل</option>
                    <option value="0" {{ old('is_active', $reportType->is_active ? '1' : '0') == '0' ? 'selected' : '' }}>معطل</option>
                </select>
            </div>
