        ...
        <div class="col-md-12">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h2>
                    ...
                    مراجعة بيانات المحافظات
                </h2>
                <a href="{{ route('governorates.import') }}" class="btn btn-outline-secondary">
            ...
            <div class="card import-preview-card mb-4">
                ...
                <div class="card-body">
                    <form action="{{ route('governorates.process-import') }}" method="POST" id="importForm" class="import-form">
                        @csrf
                        ...
                        <div class="table-responsive mb-4">
                            <table class="table mapping-table">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>
                                            <strong>{{ $field === 'name' ? 'اسم المحافظة' : $field }}</strong>
                                            @if($field === 'name')
                                        ...
                                        <td>
                                            <select name="mapping[{{ $field }}]" class="form-select" {{ $field === 'name' ? 'required' : '' }}>
                                                ...
                                                @foreach($headers as $header)
                                                    <option value="{{ $header }}" {{ $field === 'name' && str_contains(strtolower($header), 'name') ? 'selected' : '' }}>
                                                        {{ $header }}
                                                    </option>
                                                @endforeach
                                            </select>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    mappingSelects.forEach(select => {
        ...
        options.forEach(option => {
            ...
            if (fieldName === 'name' && (headerName.includes('name') || headerName.includes('اسم') || headerName.includes('محافظة'))) {
