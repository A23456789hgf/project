...
@if($allExecutions && $allExecutions->count() > 0)
    <div class="mt-4">
        ...
        <div class="table-responsive" style="max-height: 500px; overflow-y: auto;" data-procedure-id="{{ $procedure->id }}">
            <table class="table table-sm table-bordered table-hover align-middle" id="execution_tbody_{{ $procedure->id }}">
                ...
                <tbody>
                    @foreach($allExecutions as $execution)
                        @php
                            ...
                            if ($procedure->end_date && $execution->actual_finish_date_gregorian) {
                                $plannedEnd = \Carbon\Carbon::parse($procedure->end_date);
                        ...
                        @endphp
                        <tr id="exec-row-{{ $execution->id }}" data-procedure-id="{{ $procedure->id }}" data-actual-start-date="{{ $execution->actual_start_date_gregorian?->format('Y-m-d') }}" data-actual-end-date="{{ $execution->actual_finish_date_gregorian?->format('Y-m-d') }}">
                            ...
                            <td class="text-center">
                                @php
                                    $divisor = max($execution->actual_amount ?? 0, 1);
                                    $completionPct = (($execution->amount_spent ?? 0) / $divisor) * 100;
                                @endphp
                            ...
                            <td class="text-center">
                                <div class="btn-group btn-group-sm gap-1" role="group">
                                    ...
                                    @php
                                        $isDelayed = $procedure->end_date && $execution->actual_finish_date_gregorian && 
                                                    $execution->actual_finish_date_gregorian > $procedure->end_date;
                                    ...
                                    <div class="row">
                                        <div class="col-md-6 mb-3">
                                            ...
                                            <dl class="row small">
                                                ...
                                                <dd class="col-sm-6">{{ number_format((($execution->amount_spent ?? 0) / max($execution->actual_amount ?? 0, 1)) * 100, 1) }}%</dd>
                        ...
                        <!-- Delay Explanation Row (Collapse) -->
                        @php
                            $delayExplanation = $execution->delayExplanation;
                            $checkIsDelayed = $procedure->end_date && $execution->actual_finish_date_gregorian && 
                                             $execution->actual_finish_date_gregorian > $procedure->end_date;
                        @endphp
                        @if($checkIsDelayed || $delayExplanation)
                            <tr class="collapse" id="delay-row-{{ $execution->id }}">
                                <td colspan="11">
                                    <div class="p-3 bg-warning bg-opacity-10 rounded border border-warning">
                                        ...
                                        @if($delayExplanation)
                                            ...
                                            @if($delayExplanation->approval_status === 'pending')
                                                <div class="mt-3 p-3 bg-light rounded border">
                                                    <h6 class="mb-2">ملاحظات المراجعة (اختياري):</h6>
                                        ...
                                        @else
                                            <div class="d-flex align-items-center justify-content-between">
                                                <p class="text-muted mb-0">لم يتم إدخال شرح التأخير حتى الآن</p>
                                                <button type="button" 
                                                        ...
                                                        <div class="card border-primary" style="max-width: 150px;">
                                                            <div class="card-body text-center p-2">
                                                                ...
                                                                <button type="button" 
                                                                        ...
                                                                        data-doc-path="{{ $doc }}"
                                                                        data-delete-url="{{ route('projects.execution.delete-preliminary-technical', [$project, $execution->id]) }}">
                                                                    <i class="fas fa-trash"></i>
                                                                </button>
                                                                @endcan
                                                            </div>
                                                        ...
                                                        <div class="card border-success" style="max-width: 150px;">
                                                            <div class="card-body text-center p-2">
                                                                ...
                                                                <button type="button" 
                                                                        class="btn btn-xs btn-outline-danger mt-2 delete-doc"
                                                                        data-doc-path="{{ $doc }}"
                                                                        data-delete-url="{{ route('projects.execution.delete-preliminary-financial', [$project, $execution->id]) }}">
                                                                    <i class="fas fa-trash"></i>
                                                                </button>
                                                                @endcan
                                                            </div>
