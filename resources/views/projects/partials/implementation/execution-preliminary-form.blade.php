...
<div class="card-body bg-light-subtle rounded mb-3" id="add-row-card-{{ $procedure->id }}">
    @php
        ...
        $procedureBudget = $procedure->costs->sum('total') ?? 0;
        $remainingBudget = max(0, $procedureBudget - $totalAmountSpent);
    @endphp
    ...
    <div class="row mb-4 g-3">
            ...
            <div class="card border-0 bg-gradient-light shadow-sm h-100">
                <div class="card-body">
                    ...
                    <div class="display-6 fw-bold text-info" 
                         id="summary-budget-{{ $procedure->id }}"
                         data-base-value="{{ $procedureBudget }}">{{ number_format($procedureBudget, 2) }}</div>
            ...
            <div class="card border-0 bg-gradient-light shadow-sm h-100">
                <div class="card-body">
                    ...
                    <div class="display-6 fw-bold text-primary" 
                         id="summary-remaining-{{ $procedure->id }}"
                         data-base-value="{{ $remainingBudget }}">{{ number_format($remainingBudget, 2) }}</div>
                    <div class="progress mt-2" style="height: 8px;">
                        <div class="progress-bar bg-primary" 
                             id="summary-remaining-bar-{{ $procedure->id }}"
                             role="progressbar" 
                             style="width: {{ $procedureBudget > 0 ? ($remainingBudget / $procedureBudget * 100) : 0 }}%"
                             aria-valuenow="{{ $procedureBudget > 0 ? ($remainingBudget / $procedureBudget * 100) : 0 }}" 
                             aria-valuemin="0" 
        ...
        <div class="col-md-3">
            <div class="card border-0 bg-gradient-light shadow-sm h-100">
                <div class="card-body">
                    <div class="d-flex align-items-center justify-content-between mb-2">
                        <h6 class="text-muted text-uppercase small fw-bold mb-0">إجمالي الإنجاز</h6>
                        <i class="fas fa-percentage text-warning fa-lg opacity-75"></i>
                    </div>
                    <div class="display-6 fw-bold text-warning">
                        <span id="summary-completion-{{ $procedure->id }}" data-base-value="{{ $totalCompletionPct }}">{{ number_format(min($totalCompletionPct, 100), 1) }}</span>%
                    </div>
                    <div class="progress mt-2" style="height: 8px;">
                        <div class="progress-bar bg-warning" 
                             id="summary-completion-bar-{{ $procedure->id }}"
                             role="progressbar" 
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <label class="form-label">تاريخ البداية الفعلي (ميلادي) * <small class="text-muted">(مخطط: {{ $procedure->start_date?->format('d/m/Y') ?? '-' }})</small></label>
            <input type="date" 
                   class="form-control actual-start-gregorian"
                   name="actual_start_date_gregorian"
                   data-hijri-field="actual_start_hijri_display_{{ $procedure->id }}"
                   value="{{ $procedure->start_date ? $procedure->start_date->format('Y-m-d') : '' }}">
        </div>
        <div class="col-md-4">
            ...
            <input type="text" 
                   ...
                   id="actual_start_hijri_display_{{ $procedure->id }}"
                   readonly>
        </div>
        <div class="col-md-4">
    ...
    <div class="row mb-3">
        <div class="col-md-4">
            <label class="form-label">تاريخ النهاية الفعلي (ميلادي) <small class="text-muted">(مخطط: {{ $procedure->end_date?->format('d/m/Y') ?? '-' }})</small></label>
            <input type="date" 
                   class="form-control actual-finish-gregorian"
                   name="actual_finish_date_gregorian"
                   data-hijri-field="actual_finish_hijri_display_{{ $procedure->id }}"
                   value="{{ $procedure->end_date ? $procedure->end_date->format('Y-m-d') : '' }}">
        </div>
        <div class="col-md-4">
            ...
            <input type="text" 
                   ...
                   id="actual_finish_hijri_display_{{ $procedure->id }}"
                   readonly>
        </div>
        <div class="col-md-4">
        ...
        <div class="col-md-4">
            ...
            <div class="input-group">
                <input type="number" 
                       ...
                       step="0.01" 
                       min="0" 
                       value="{{ number_format($procedureBudget, 2, '.', '') }}">
        ...
        <div class="col-md-4">
            ...
            <div class="input-group">
                <input type="number" 
                       ...
                       id="remaining-amount-display-{{ $procedure->id }}"
                       readonly
    ...
    <!-- Budget Warning -->
    <div class="alert alert-warning alert-dismissible fade show mb-3" id="budget-warning-{{ $procedure->id }}" style="display: none;">
        <i class="fas fa-exclamation-triangle me-2"></i>
        <strong>تنبيه:</strong> المبلغ المنفق يتجاوز المبلغ المتبقي من الميزانية!
        <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
    </div>

    <input type="hidden" name="planned_amount" value="{{ $procedureBudget }}">
    <input type="hidden" name="preliminary_procedure_id" value="{{ $procedure->id }}">

    <!-- Delay Explanation Section -->
    <div class="row mb-3">
        <div class="col-12">
            <div class="card border-warning" id="delay-section-preliminary-{{ $procedure->id }}">
                <div class="card-header bg-warning bg-opacity-10 py-2">
                    <h6 class="m-0 font-weight-bold text-warning">
                        <i class="fas fa-exclamation-triangle me-2"></i>شرح التأخير (سيظهر تلقائياً عند اكتشاف تأخير)
                    </h6>
                </div>
                <div class="card-body" style="display: none;" id="delay-content-preliminary-{{ $procedure->id }}">
                    <div class="mb-3">
                    ...
                    <div id="delay-attachments-preview-preliminary-{{ $procedure->id }}" class="d-flex flex-wrap gap-2">
        ...
        <div class="col-12">
            <div class="card border-primary">
                ...
                <div class="card-body">
                    ...
                    <div id="technical-docs-preview-{{ $procedure->id }}" class="d-flex flex-wrap gap-2">
                        <!-- Preview will be displayed here -->
                    </div>
                </div>
            </div>
        ...
        <div class="col-12">
            <div class="card border-success">
                ...
                <div class="card-body">
                    ...
                    <div id="financial-docs-preview-{{ $procedure->id }}" class="d-flex flex-wrap gap-2">
                        <!-- Preview will be displayed here -->
                    </div>
                </div>
            </div>
