...
<div class="app-layout-wrapper">
    ...
    <div class="content-scrollable-area" id="mainScrollContainer">
        <!-- Diagnostic Panel (Only visible for investigation) -->
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const FormManager = {
        ...
        setupFormObservers: function() {
            // Watch for any changes in the form to set isDirty
            ...
            if (form) {
                ...
                // For Select2 and other plugins that trigger 'change'
                $(form).on('change', 'select, input', () => {
                    this.isDirty = true;
                });
            }
        },
        ...
        initAutoSave: function() {
            ...
            this.autoSaveInterval = setInterval(() => {
                if (this.isDirty && !this.saveInProgress && this.currentStep < this.totalSteps) {
                    ...
                    this.saveStep(null, false, true); // Silent, background save
                }
        ...
        loadDraftSettings: function() {
            ...
            if (isDraftField && isDraftField.value === 'true') {
                ...
                if (projectIdField && projectIdField.value) {
                    this.projectId = projectIdField.value;
                    this.debug(`Project ID set: ${this.projectId}`);
                }
                
                // Log project ID if available
                if (projectIdField && projectIdField.value) {
                    this.debug(`Project ID: ${projectIdField.value}`);
                }
            }
        },
        ...
        nextStep: function() {
            // We always attempt to save as draft before moving
            const isStrict = true;
            const isValid = this.validateCurrentStep(isStrict);
            
            const statusInput = document.querySelector('input[name="status"]');
            if (statusInput) statusInput.value = 'draft';

            // We pass 'next' as direction to trigger backend validation
            this.saveStep(() => {
                // If it was valid, we move to the next step
                if (isValid) {
                    ...
                    this.debug('Navigation blocked: Current step has validation errors');
                    AppUtils.Utils.showToast('تم حفظ البيانات كمسودة، ولكن لا يمكن الانتقال للمرحلة التالية لوجود أخطاء.', 'warning');
                }
            }, !isValid, false, 'next'); // Show toast only if invalid (to show errors), not background
        },
        ...
        prevStep: function() {
            if (this.currentStep > 1) {
                const statusInput = document.querySelector('input[name="status"]');
                if (statusInput) statusInput.value = 'draft';
                // Save silently as background (no errors shown, not blocking)
                this.saveStep(() => {
                    this.currentStep--;
                    this.updateUI();
                    this.updateProgressBar();
        ...
        goToStep: function(step) {
            if (step >= 1 && step <= this.totalSteps) {
                ...
                const isValid = direction === 'next' ? this.validateCurrentStep(true) : true;

                const statusInput = document.querySelector('input[name="status"]');
                if (statusInput) statusInput.value = 'draft';

                this.saveStep(() => {
                    // Only move forward if valid, always move backward
                    if (direction === 'back' || isValid) {
                        ...
                        this.debug('Navigation blocked: Target step is ahead and current step is invalid');
                        AppUtils.Utils.showToast('لا يمكن الانتقال للأمام لوجود أخطاء في البيانات الحالية.', 'warning');
                    }
                }, !isValid, false, direction);
            }
        },
        ...
        populateExecutiveActivityDropdowns: function() {
            ...
            validateStep3: function(isStrict = false) {
                ...
                const supervisingTable = document.querySelector('#supervisingAuthoritiesTable tbody');
                if (supervisingTable) {
            ...
            normalizeFormData: function(formData) {
                // We previously deleted '0' values here, but '0' represents "All" 
                // in our cascading dropdowns and must be sent to the server.
                return formData;
            },
            
            saveStep: function(callback, showErrors = true, isBackground = false, direction = null) {
                ...
                // Add navigation direction to trigger server-side validation if moving forward
                if (direction) {
                ...
                .then(({ ok, status, data }) => {
                    ...
                    if (ok || status === 422) {
                        // Data was saved (even if validation failed on 422)
                        this.updateSaveIndicator('saved');
                        
                        if (status === 422 && showErrors) {
                ...
                .catch(error => {
                    ...
                    if (!isBackground) {
                        AppUtils.Utils.showToast('فشل في حفظ البيانات: ' + (error.message || 'خطأ غير معروف'), 'error');
                    }
            ...
            saveDraft: function() {
                ...
                if (form) {
                    ...
                    this.saveStep(() => {
                        // Redirect to projects index after successful draft save
            ...
            submitForm: function() {
                if (this.validateCurrentStep(true)) { // Strict validation for final submission
                    ...
                    if (form) {
                        ...
                        AppUtils.Utils.showToast('جاري إرسال المشروع...', 'info');
                        this.showLoadingState();
                        this.saveStep(() => {
    ...
    window.checkEntityData = function() {
        ...
        const supervisingRows = document.querySelectorAll('#supervisingAuthoritiesTable tbody tr:not(.project-empty-row)');
    ...
    // 🔬 New Diagnostic Panel Controller
    window.runDiagnostics = function() {
        ...
        let report = '';
        report += `• Current Step: ${FormManager.currentStep}\n`;
        report += `• Project ID: ${FormManager.projectId || 'None'}\n`;
        ...
        const objectives = {
            count: document.querySelectorAll('.objective-row').length,
            results: document.querySelectorAll('.result-row').length,
            outputs: document.querySelectorAll('.project-output-item').length
        };
        
        report += `• DOM Objectives: ${objectives.count} Obj, ${objectives.results} Res, ${objectives.outputs} Out\n`;
        ...
        console.log('🔬 Extended Diagnostic Report:', { entities, objectives, state: FormManager });
    };

    // Auto-run diagnostics on step 3
    ...
    // Toggle diagnostic panel with Ctrl+Alt+D
...
<script>
    // Bind existing project date fields
    document.addEventListener('DOMContentLoaded', function() {
        if (document.getElementById('start_date_gregorian')) {
            bindGregorianToHijri("start_date_gregorian", "start_date_hijri");
        }
        if (document.getElementById('end_date_gregorian')) {
            bindGregorianToHijri("end_date_gregorian", "end_date_hijri");
        }
        
        // Bind requested actual date fields (if they exist)
        if (document.getElementById('actual_start_g')) {
            bindGregorianToHijri("actual_start_g", "actual_start_h");
        }
        if (document.getElementById('actual_end_g')) {
            bindGregorianToHijri("actual_end_g", "actual_end_h");
        }
    });
</script>
