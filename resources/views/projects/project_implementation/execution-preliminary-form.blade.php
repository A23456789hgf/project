...
<div class="card-body bg-light-subtle rounded mb-3" id="add-row-card-{{ $procedure->id }}">
    ...
    <div class="row mb-3">
        <div class="col-12">
            <div class="card border-warning" id="delay-section-preliminary">
                <div class="card-header bg-warning bg-opacity-10 py-2">
                    <h6 class="m-0 font-weight-bold text-warning">
                        <i class="fas fa-exclamation-triangle me-2"></i>شرح التأخير (سيظهر تلقائياً عند اكتشاف تأخير)
                    </h6>
                </div>
                <div class="card-body" style="display: none;" id="delay-content-preliminary">
                    <div class="mb-3">
                    ...
                    <div id="delay-attachments-preview-preliminary" class="d-flex flex-wrap gap-2">
...
document.addEventListener('DOMContentLoaded', function() {
    const plannedEndDate = '{{ $procedure->end_date ? $procedure->end_date->format("Y-m-d") : "" }}';
    
    function checkForDelay() {
        const finishDateInput = document.querySelector('.actual-finish-gregorian');
        const delaySection = document.getElementById('delay-content-preliminary');
    ...
    if (techDocsInput) {
        techDocsInput.addEventListener('change', function() {
            updateDocumentPreview(this, `technical-docs-preview-{{ $procedure->id }}`);
        });
    }
    ...
    if (finDocsInput) {
        finDocsInput.addEventListener('change', function() {
            updateDocumentPreview(this, `financial-docs-preview-{{ $procedure->id }}`);
        });
    }
    ...
    if (delayAttachmentsInput) {
        delayAttachmentsInput.addEventListener('change', function() {
            updateDocumentPreview(this, 'delay-attachments-preview-preliminary');
        });
    }
