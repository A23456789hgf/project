    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card border-0 shadow-lg">
                <div class="card-header bg-gradient-sidebar text-white py-3 d-flex justify-content-between align-items-center">
                    <h5 class="mb-0"><i class="fas fa-file-signature me-2"></i>إنشاء مذكرة إدارية جديدة</h5>
                    <a href="{{ route('memoirs.index') }}" class="btn btn-light btn-sm shadow-sm">
                        <i class="fas fa-arrow-right me-1"></i>العودة للقائمة
                    </a>
                </div>
                <div class="card-body p-4">
                    <form action="{{ route('memoirs.store') }}" method="POST">
                        @csrf
                        ...
                        <div class="row g-4 mb-4">
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="to" id="to" class="form-control shadow-sm @error('to') is-invalid @enderror" 
                                       placeholder="مثلاً: مدير عام الادارة المالية" value="{{ old('to') }}" required>
                                @error('to')
                            ...
                            <div class="col-md-12">
                                ...
                                <input type="text" name="subject" id="subject" class="form-control shadow-sm @error('subject') is-invalid @enderror" 
                                       placeholder="عنوان الموضوع باختصار" value="{{ old('subject') }}" required>
                                @error('subject')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="gregorian_date" id="gregorian_date" class="form-control shadow-sm @error('gregorian_date') is-invalid @enderror" 
                                       value="{{ old('gregorian_date', $gregorianDate) }}" required>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="hijri_date" id="hijri_date" class="form-control shadow-sm border-0 bg-light @error('hijri_date') is-invalid @enderror" 
                                       value="{{ old('hijri_date', $hijriDate) }}" required readonly>
                                <small class="text-muted italic">يتم تحديث التاريخ الهجري تلقائياً بناءً على التاريخ الميلادي.</small>
                                @error('hijri_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="body" id="body" class="form-control shadow-sm @error('body') is-invalid @enderror" 
                                          rows="12" placeholder="اكتب نص المذكرة هنا بالتفصيل..." required>{{ old('body') }}</textarea>
                                @error('body')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="d-flex gap-4 mb-3">
                                            <div class="form-check custom-radio">
                                                <input class="form-check-input" type="radio" name="is_project_related" id="is_project_no" value="no" checked>
                                                <label class="form-check-label px-2" for="is_project_no">لا</label>
                                            </div>
                                            <div class="form-check custom-radio">
                                                <input class="form-check-input" type="radio" name="is_project_related" id="is_project_yes" value="yes">
                                                <label class="form-check-label px-2" for="is_project_yes">نعم</label>
                                            </div>
                                        </div>

                                        <div id="project_selection_container" style="display: none;" class="animate__animated animate__fadeIn">
                                            <label for="project_id" class="form-label fw-bold small text-primary">اختر المشروع المرتبط <span class="text-danger">*</span></label>
                                            ...
                                            <div id="project_number_display" class="mt-2" style="display: none;">
                                                <div class="d-flex align-items-center p-2 bg-white rounded border border-primary-subtle shadow-sm">
                                                    ...
                                                    <div>
                                                        ...
                                                        <span id="project_form_number_text" class="fw-bold text-dark">-</span>
                                                    </div>
                                                </div>
                                            </div>
                        ...
                        <div class="d-grid gap-2 d-md-flex justify-content-md-end">
                            <button type="reset" class="btn btn-outline-secondary px-4 me-md-2 shadow-sm">
                                <i class="fas fa-undo me-1"></i>تفريغ الحقول
                            </button>
                            <button type="submit" class="btn btn-primary px-5 shadow-sm">
                                <i class="fas fa-save me-1"></i>حفظ المذكرة
                            </button>
                        </div>
                    </form>
                </div>
                <div class="card-footer bg-light border-0 py-3 text-muted small">
                    <i class="fas fa-info-circle me-1 text-primary"></i> سيتم توليد رقم المذكرة تلقائياً بمجرد الحفظ، وسيتم ربط المذكرة بالجهة الحالية: <strong>{{ auth()->user()->entity?->name }}</strong>
                </div>
            </div>
        </div>
    </div>
...
<script>
    $(document).ready(function() {
        ...
        // Maintain state on validation error
        if ($('input[name="is_project_related"]:checked').val() === 'yes') {
            $('#project_selection_container').show();
            $('#project_id').prop('required', true);
            
            const initialFormNumber = $('#project_id').find(':selected').data('form-number');
            if (initialFormNumber) {
                $('#project_form_number_text').text(initialFormNumber);
                $('#project_number_display').show();
            }
        }

        // Date sync logic (Placeholder from existing template)
        $('#gregorian_date').on('change', function() {
            // Existing logic or AJAX call to update hijri_date if needed
        });
    });
</script>
@endpush
